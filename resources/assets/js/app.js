...
$(document).ready(function() {
	$('#boxersTable').DataTable({paging: false});
});
...
function fight(a, b) {
    ...
    for (var i = 1; i <= 12; i++) {
        ...
        if (aovr >= bovr) {
            around++;
            if ((aovr - bovr > 19) && (b.chin < 73) && (a.power > 69)) {
                aknockout++;
                round = i;
                break;
            }
            if ((aovr - bovr > 14) && (a.power > 61) && (b.chin < 67)) {
                aknockdown++;
                b.chin - 3;
                b.defense - 3;
            }
            ...
            bround++;
            if ((bovr - aovr > 19) && (a.chin < 73) && (b.power > 69)) {
                bknockout++;
                round = i;
                break;
            }
            if ((bovr - aovr > 14) && (b.power > 61) && (a.chin < 67)) {
                bknockdown++;
                a.chin - 3;
                a.defense - 3;
            }
...
function generateAWinner(a, b) {
    ...
    } else if (bknockout > 0 || bcard > acard) {
        winner = b.name;
        winnerimage = b.image;
        winnermethod = cleanScore(scores);
    } else {
...
$("#round1fight1").on("click", function() {
        if (boxers) {
            generateAWinner(boxers[0], boxers[7]);
            if (aknockout >= 1) {
                $("#round2seed1image").attr("src", boxers[0].image);
                document.getElementById("round2seed1name").innerText = boxers[0].name;
                document.getElementById("round2seed1method").innerText = boxers[0].name + ' knocked out ' + boxers[7].name + ' in round ' + round;
            } else if (bknockout >= 1) {
                $("#round2seed1image").attr("src", boxers[7].image);
                document.getElementById("round2seed1name").innerText = boxers[7].name;
                document.getElementById("round2seed1method").innerText = boxers[7].name + ' knocked out ' + boxers[0].name + ' in round ' + round;
            } else {
        ...
        if (boxers[0]["name"] == winner) {
            round2seed1 = boxers[0];
        } else {
            round2seed1 = boxers[7];
        }
        resetVariables();
...
$("#round1fight2").on("click", function() {
        if (boxers) {
            generateAWinner(boxers[1], boxers[6]);
            if (aknockout >= 1) {
                $("#round2seed2image").attr("src", boxers[1].image);
                document.getElementById("round2seed2name").innerText = boxers[1].name;
                document.getElementById("round2seed2method").innerText = boxers[1].name + ' knocked out ' + boxers[6].name + ' in round ' + round;
            } else if (bknockout >= 1) {
                $("#round2seed2image").attr("src", boxers[6].image);
                document.getElementById("round2seed2name").innerText = boxers[6].name;
                document.getElementById("round2seed2method").innerText = boxers[6].name + ' knocked out ' + boxers[1].name + ' in round ' + round;
            } else {
...
$("#round1fight3").on("click", function() {
    if (boxers) {
        generateAWinner(boxers[2], boxers[5]);
        if (aknockout >= 1) {
            $("#round2seed3image").attr("src", boxers[2].image);
            document.getElementById("round2seed3name").innerText = boxers[2].name;
            document.getElementById("round2seed3method").innerText = boxers[2].name + ' knocked out ' + boxers[5].name + ' in round ' + round;
        } else if (bknockout >= 1) {
            $("#round2seed3image").attr("src", boxers[5].image);
            document.getElementById("round2seed3name").innerText = boxers[5].name;
            document.getElementById("round2seed3method").innerText = boxers[5].name + ' knocked out ' + boxers[2].name + ' in round ' + round;
        } else {
...
$("#round1fight4").on("click", function() {
    if (boxers) {
        generateAWinner(boxers[3], boxers[4]);
        if (aknockout >= 1) {
            $("#round2seed4image").attr("src", boxers[3].image);
            document.getElementById("round2seed4name").innerText = boxers[3].name;
            document.getElementById("round2seed4method").innerText = boxers[3].name + ' knocked out ' + boxers[4].name + ' in round ' + round;
        } else if (bknockout >= 1) {
            $("#round2seed4image").attr("src", boxers[4].image);
            document.getElementById("round2seed4name").innerText = boxers[4].name;
            document.getElementById("round2seed4method").innerText = boxers[4].name + ' knocked out ' + boxers[3].name + ' in round ' + round;
        } else {
